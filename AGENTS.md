...
This repository is a monorepo containing an i18n conversion tool with two top-level modules:

- **`core/`** — Python CLI tool for converting i18n JSON files to Excel and back. Runs independently.
- **`gui/`** — Desktop GUI wrapper (Tauri + React, planned for post-v1.0.0).

```text
i18n-tool/
├── core/                    # Python CLI (independent)
│   ├── pyproject.toml
│   ├── src/i18n_tools/      # Main source package
│   ├── tests/               # Test suite
│   ├── data/                # Sample config
│   ├── docs/                # User-facing docs (j2e, e2j)
│   └── scripts/             # Build/release scripts
├── gui/                     # Desktop GUI (Tauri + React, future)
├── docs/
│   └── plan/                # Project-wide planning docs
│       └── PDCA.md
└── AGENT.md                 # This file
```
